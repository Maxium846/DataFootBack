...
public class MatchImportService {
    ...
    public MatchImportService(LeagueRepository leagueRepository,
    ...
    public String generateCalendarFromApiFootball(Long leagueId,int season) {
        ...
        for (ResponseApItemDtoMatch dto : responseApiMatch.getResponse()){
            ...
            Team teamHome = teamRepository.findByApiFootballTeamId(dto.getTeams().getHome().getId()).orElseThrow(()->new TeamNotFoundException("L'équipe n'existe pas en base"));
            Team teamAway = teamRepository.findByApiFootballTeamId(dto.getTeams().getAway().getId()).orElseThrow(()->new TeamNotFoundException("L'équipe n'existe pas en base"));
